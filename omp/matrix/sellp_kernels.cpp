...
template <typename ValueType, typename IndexType>
int get_spmm_version(const matrix::Sellp<ValueType, IndexType>* a)
{
...
// SELLP_v1: Gustavson's algorithm + parallelization over slices and rows.
template <typename ValueType, typename IndexType>
void spmm_v1(std::shared_ptr<const OmpExecutor> exec,
             ...
             matrix::view::dense<ValueType> c)
{
    const auto slice_lengths = a->get_const_slice_lengths();
    const auto slice_sets = a->get_const_slice_sets();
    const auto slice_size = a->get_slice_size();
...
// SELLP_v2: Gustavson's algorithm + parallelization over slices and rows + SIMD
// over dense columns.
template <typename ValueType, typename IndexType>
