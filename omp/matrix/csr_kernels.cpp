...
// CSR_v1: Gustavson's algorithm. Sequential implementation of spmm.
template <typename MatrixValueType, typename InputValueType,
          typename OutputValueType, typename IndexType>
void spmm_v1([[maybe_unused]] std::shared_ptr<const OmpExecutor> exec,
             const matrix::Csr<MatrixValueType, IndexType>* a,
             matrix::view::dense<const InputValueType> b,
             matrix::view::dense<OutputValueType> c)
{
...
// CSR_v2: Gustavson's algorithm + parallelization over rows.
template <typename MatrixValueType, typename InputValueType,
          typename OutputValueType, typename IndexType>
void spmm_v2([[maybe_unused]] std::shared_ptr<const OmpExecutor> exec,
             const matrix::Csr<MatrixValueType, IndexType>* a,
             matrix::view::dense<const InputValueType> b,
             matrix::view::dense<OutputValueType> c)
{
...
// CSR_v3: Gustavson's algorithm + parallelization over rows + SIMD over dense
// columns.
template <typename MatrixValueType, typename InputValueType,
          typename OutputValueType, typename IndexType>
void spmm_v3([[maybe_unused]] std::shared_ptr<const OmpExecutor> exec,
             const matrix::Csr<MatrixValueType, IndexType>* a,
             matrix::view::dense<const InputValueType> b,
             matrix::view::dense<OutputValueType> c)
{
...
int parse_omp_csr_spmm_version(const std::string& name)
{
    if (name == "spmm_dynamic") {
        return 2;
    }
    static constexpr char prefix[] = "spmm_v";
    ...
    for (std::size_t i = plen; i < name.size(); ++i) {
        ...
        version = version * 10 + static_cast<int>(c - '0');
...
{
    const auto version =
        parse_omp_csr_spmm_version(a->get_strategy()->get_name());
    switch (version) {
    case 1:
        spmm_v1(exec, a, b, c);
        break;
