...
protected:
    ...
    void assert_variant_matches_ref(int variant)
    {
        ...
        dmtx->set_strategy(
            std::make_shared<typename Mtx::omp_spmm_variant>(variant));
...
TEST_F(CsrSpmm, SpmmV1IsEquivalentToRef)
{
    set_up_apply_data();
...
TEST_F(CsrSpmm, SpmmV2IsEquivalentToRef)
{
    set_up_apply_data();
...
TEST_F(CsrSpmm, SpmmV3IsEquivalentToRef)
{
    set_up_apply_data();
    assert_variant_matches_ref(3);
}
